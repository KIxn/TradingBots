...
if __name__ == "__main__":
    ...
    settings_choice.selectbox(
        "Use Settings File", ["Yes", "No"], index=None, placeholder="Select an option", key="settings_file"
    )

    alerting_option = alert_listener.selectbox(
        "Alert Listener", ["Discord"], index=None, placeholder="Select an alerting option"
    )
    ...
    settings.selectbox("Make Trades", ["Yes", "No"], index=None, placeholder="Select an option", key="make_trades")
    ...
    with tab_backtest:
        ...
        bt_symbol = col_sym.selectbox("Symbol", streamlit.session_state["symbols"], index=None, key="bt_symbol")
        bt_timeframe = col_tf.selectbox(
            "Timeframe", streamlit.session_state["timeframes"], index=None, key="bt_timeframe"
        )
        bt_strategy = col_strat.selectbox(
            "Strategy", streamlit.session_state["strategies"], index=None, key="bt_strategy"
        )
        ...
        if bt_timeframe in small_timeframes and date_range_days > 180:
            streamlit.warning(
                f"⚠️ {bt_timeframe} timeframe over {date_range_days} days may return too much data or exceed MT5 history limits. Consider using H1 or higher for ranges over 6 months."
            )
            ...
            if not bt_symbol or not bt_timeframe or not bt_strategy:
                ...
                with streamlit.status("Running backtest...", expanded=True) as status:
                    try:
                        ...
                        df = metatrader_interface.get_historic_data_range(bt_symbol, bt_timeframe, date_from, date_to)
                        ...
                        with streamlit.expander("Full Stats"):
                            streamlit.dataframe(safe_series(stats.drop(["_equity_curve", "_trades"])), width="stretch")
                        ...
                        streamlit.components.v1.html(
                            pathlib.Path(chart_path).read_text(encoding="utf-8"), height=600, scrolling=True
                        )
                    except Exception as e:
                        import traceback
    ...
    with tab_live:
        ...
        if col_start.button("▶ Start Trading", use_container_width=True, disabled=streamlit.session_state.running):
            if not all([streamlit.session_state.get(k) for k in ["platform", "symbol", "timeframe", "strategy"]]):
                ...
                streamlit.session_state["loop_platform"] = (
                    "MetaTrader5"
                    if streamlit.session_state["platform"] == "MetaTrader 5"
                    else streamlit.session_state["platform"]
                )
                    ...
                    with live_result.container():
                        ...
                        if make_trades and decision != "HOLD":
                            ...
                            try:
                                ...
                                streamlit.success(
                                    f"✅ Order placed | Ticket: {order_result.order} | {decision} {symbol} @ {price:.5f} | SL: {sl:.5f} | TP: {exit_price:.5f}"
                                )
